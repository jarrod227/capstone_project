...
def run_ml_mode(source, calibrator=None, kalman=None, keyboard_overlay=None):
    ...
    for packet in source.stream():
        ...
        if prediction and prediction != "idle":
            ...
            # Step 2: only while locked, head tilt triggers scroll.
...
Examples:
  ...
  python main.py --simulate --mode ml                  # ML mode with simulator
    ...
    print("    Double click:   Triple blink (3 quick blinks)")
    ...
    if args.simulate:
        ...
        print("    L/R + N (tap x2) → look left/right + double nod (center cursor)")
